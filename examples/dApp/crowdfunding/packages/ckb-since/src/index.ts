...
export class Since {
    ...
    cmp(other: Since): Ordering | null {
        ...
        switch (a.kind) {
            case "BlockNumber":
            case "Timestamp":
                if (a.value < (b as typeof a).value)
                    return -1;
                else if (a.value > (b as typeof a).value)
                    return 1;
                else
                    return 0;
...
export class EpochNumberWithFraction {
    ...
    static readonly NUMBER_MAXIMUM_VALUE = 1n << EpochNumberWithFraction.NUMBER_BITS;
    static readonly NUMBER_MASK = EpochNumberWithFraction.NUMBER_MAXIMUM_VALUE - 1n;
    static readonly INDEX_OFFSET = EpochNumberWithFraction.NUMBER_BITS;
    static readonly INDEX_BITS = 16n;
    static readonly INDEX_MAXIMUM_VALUE = 1n << EpochNumberWithFraction.INDEX_BITS;
    static readonly INDEX_MASK = EpochNumberWithFraction.INDEX_MAXIMUM_VALUE - 1n;
    static readonly LENGTH_OFFSET = EpochNumberWithFraction.NUMBER_BITS + EpochNumberWithFraction.INDEX_BITS;
    static readonly LENGTH_BITS = 16n;
    static readonly LENGTH_MAXIMUM_VALUE = 1n << EpochNumberWithFraction.LENGTH_BITS;
    static readonly LENGTH_MASK = EpochNumberWithFraction.LENGTH_MAXIMUM_VALUE - 1n;
    ...
    static new_unchecked(number: bigint, index: bigint, length: bigint): EpochNumberWithFraction {
    ...
    static create(number: bigint, index: bigint, length: bigint): EpochNumberWithFraction | null {
        if (number < EpochNumberWithFraction.NUMBER_MAXIMUM_VALUE
    ...
    add(rhs: EpochNumberWithFraction): EpochNumberWithFraction | null {
        ...
        const toBig = (x: number | bigint) => (typeof x === "bigint" ? x : BigInt(x));
        ...
        if (numerator < 0n || numerator > U64_MAX)
            return null;
        if (denominator < 1n || denominator > U64_MAX)
            return null;
